...
The next M
 lines each contain two integers describing a segment in Bessie's journey,
 giving the length and also the speed at which Bessie was driving.

OUTPUT FORMAT (file speeding.out):
Please output a single line containing the maximum amount over
 the speed limit Bessie drove during any part of her journey.
...
and 40 miles at 40 miles per hour). During her first segment, she is slightly over the speed limit,
but her last segment is the worst infraction, during part of which she is 5 miles per hour over the
...
int main()
{
  ...
  vector<int> segmentsN(N);
  vector<int> speedLimitN(N);
  vector<int> segmentsM(M);
  vector<int> speedLimitM(M);
  ...
  for (int i = 0; i < N; i++)
  {
    for (int j = 0; j < segmentsN[i]; j++)
    {
      roadLimit[pos] = speedLimitN[i];
      pos++;
    }
  }
  ...
  for (int i = 0; i < M; i++)
  {
    for (int j = 0; j < segmentsM[i]; j++)
    {
      bessieSpeed[pos] = speedLimitM[i];
      pos++;
    }
  }
  ...
  for (int i = 0; i < 100; i++)
  {
    maxOver = max(maxOver, bessieSpeed[i] - roadLimit[i]);
  }
